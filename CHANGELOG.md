...
v0.6.0 曾误发布（功能正确但版本号不符预期），且 proxy 缓存不可删除，故追加 v0.6.1。
...
- **v0.4.0 和 v0.5.0 已通过 go.mod `retract` 撤回**：这两个版本默认开启循环依赖检测，会拦截本可正常工作的指针循环依赖。请升级到 v0.6.1 及以上。
- v0.6.0 因版本号规划问题跳过，功能与 v0.5.1/v0.6.1 等价。

## [0.5.1] - 2026-08-09

与 v0.6.1 内容相同（循环依赖检测改为 opt-in）。v0.4.0/v0.5.0 的回归修正版。
